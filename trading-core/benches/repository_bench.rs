...
use trading_core::data::{
    ...
    cache::{TieredCache, TickDataCache},
    types::{TickData, TradeSide, TickQuery, DataResult},
};
use dotenv::dotenv;

fn create_test_tick(symbol: &str, price: &str, trade_id: &str, timestamp: DateTime<Utc>) -> TickData {
    TickData::new(
        timestamp,
        symbol.to_string(),
...
async fn setup_repository() -> DataResult<TickDataRepository> {
    ...
    let cache = TieredCache::new((1000, 300), (&redis_url, 10000, 3600))
        .await?;
...
fn repository_benchmarks(c: &mut Criterion) {
    ...
    let repo = rt.block_on(setup_repository()).expect("Failed to setup repository");
    let cache = rt.block_on(setup_cache()).expect("Failed to setup cache");
    let pool = repo.get_pool();
    rt.block_on(cleanup_database(pool, symbol)).expect("Failed to cleanup database");
    ...
    c.bench_function("get_historical_data_for_backtest", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            repo.get_historical_data_for_backtest(black_box(symbol), start_time, end_time, Some(100))
                .await
                .unwrap();
        });
    });
    ...
    c.bench_function("cache_get_recent_ticks", |b| {
        b.to_async(&rt).iter(|| async {
            cache.get_recent_ticks(black_box(symbol), 100).await.unwrap();
        });
    });

    // Cleanup after benchmarks
    rt.block_on(cleanup_database(pool, symbol)).expect("Failed to cleanup database");
    rt.block_on(cache.clear_symbol(symbol)).expect("Failed to clear cache");
}
