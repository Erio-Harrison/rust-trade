...
impl BacktestMetrics {
    ...
    pub fn calculate_sortino_ratio(returns: &[Decimal], risk_free_rate: Decimal, target_return: Decimal) -> Decimal {
        if returns.is_empty() {
            return Decimal::ZERO;
        }
    ...
    pub fn calculate_information_ratio(portfolio_returns: &[Decimal], benchmark_returns: &[Decimal]) -> Decimal {
    ...
    pub fn calculate_profit_factor(trades: &[crate::backtest::portfolio::Trade]) -> Decimal {
        let (total_profit, total_loss) = trades.iter()
            .filter_map(|trade| trade.realized_pnl)
            .fold((Decimal::ZERO, Decimal::ZERO), |(profit, loss), pnl| {
                if pnl > Decimal::ZERO {
                    (profit + pnl, loss)
                } else {
                    (profit, loss - pnl)
        ...
        if total_loss == Decimal::ZERO {
            return if total_profit > Decimal::ZERO { Decimal::MAX } else { Decimal::ZERO };
        }
