...
impl Portfolio {
    ...
    pub fn execute_buy(&mut self, symbol: String, quantity: Decimal, price: Decimal) -> Result<(), String> {
        let cost = quantity * price;
        let commission = cost * self.commission_rate;
        let total_cost = cost + commission;
        
        if total_cost > self.cash {
            return Err(format!("Insufficient funds: need ${}, available ${}", total_cost, self.cash));
        }
    ...
    pub fn execute_sell(&mut self, symbol: String, quantity: Decimal, price: Decimal) -> Result<(), String> {
        let position = self.positions.get_mut(&symbol)
            .ok_or("No position to sell")?;
        
        if quantity > position.quantity {
            return Err(format!("Insufficient position: need {}, available {}", quantity, position.quantity));
        }
        ...
        for trade in &self.trades {
            match trade.side {
                TradeSide::Buy => {
                    ...
                    let (curr_qty, curr_avg) = running_positions.get(&trade.symbol).unwrap_or(&(Decimal::ZERO, Decimal::ZERO));
