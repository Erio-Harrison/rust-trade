...
async fn run_backtest_interactive(
    ...
    if temp_strategy.supports_ohlc() {
        if let Some(timeframe) = temp_strategy.preferred_timeframe() {
            println!("\n🔄 Strategy supports OHLC, using {} timeframe for better performance", timeframe.as_str());
            ...
            match repository.generate_recent_ohlc_for_backtest(&symbol, timeframe, candle_count).await {
                Ok(ohlc_data) if !ohlc_data.is_empty() => {
                    println!("✅ Loaded {} OHLC candles", ohlc_data.len());
                    println!("📅 Data range: {} to {}", 
                             ohlc_data.first().unwrap().timestamp.format("%Y-%m-%d %H:%M:%S"), 
                             ohlc_data.last().unwrap().timestamp.format("%Y-%m-%d %H:%M:%S"));
                ...
                Ok(_) => {
                    println!("⚠️ No OHLC data available for timeframe {}, falling back to tick data", timeframe.as_str());
                },
                Err(e) => {
                    println!("⚠️ OHLC generation failed: {}, falling back to tick data", e);
                }
            }
        }
    }
    ...
    println!("\n🔍 Loading historical tick data: {} latest {} records...", symbol, data_count);
